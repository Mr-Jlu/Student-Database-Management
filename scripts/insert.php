            ...
            if((preg_match("/^[a-zA-Z ]*$/", $f_name)) && (preg_match("/^[a-zA-Z ]*$/", $l_name)) &&
               (filter_var($email, FILTER_VALIDATE_EMAIL)) && (preg_match('/^\S*(?=\S{7,15})(?=\S*[a-z])(?=\S*[A-Z])(?=\S*[\d])\S*$/', $pass_word))){
                ...
                if($query){
                        ...
                        $msg = "Congrats ".$f_name." on registering your Account On Student Database Management <br><br><br> 
                        Click the link below to verify your email using our secure server: <a href='http://localhost/SDM/user/user_activation.php?key=".$user_activation_key."'>http://localhost/SDM/user/user_activation.php?key=".$user_activation_key." </a> "." <br><br><br><br>If clicking the link doesn't seem to work, you can copy and paste the link into your browser's address window, or retype it there. Once you have returned to SDM, we will give instructions for verifying your email.<br><br>
                        SDM will never email you and ask you to disclose or verify your SDM password. If you receive a suspicious email with a link to update your account information, do not click on the link--instead, report the email to SDM for investigation. Thanks for visiting SDM!";
